...
export default function Profile() {
  ...
  const handleCheckboxChange = (projectId, stepId, subStep) => {
    setCompletedSteps((prev) => {
      ...
      if (!updated[projectId][stepId]) updated[projectId][stepId] = new Set();
      if (updated[projectId][stepId].has(subStep)) {
        updated[projectId][stepId].delete(subStep);
      } else {
        updated[projectId][stepId].add(subStep);
      }
      return { ...updated };
    });
  };
  ...
  const handleSubmit = (projectId) => {
    ...
    axios
      .post("http://localhost:5000/api/submitCompletedSteps", {
      userId,
      projectId,
                        ...
                        {step.subSteps.map((subStep, idx) => (
                          <li key={idx} className="flex items-center space-x-2">
                            <input
                              type="checkbox"
                              checked={
                                completedSteps[project._id]?.[step._id]?.has(
                                  subStep
                                ) || false
                              }
                              onChange={() =>
